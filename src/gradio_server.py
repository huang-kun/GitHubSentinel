...
reddit_client = RedditClient() # Reddit Client
...
def generate_reddit_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    if model_type == 'openai' and model_name in ["gpt-4o", "gpt-4o-mini"]:
        for feed in feeds:
    ...
    # testing
    # file_path = reddit_client.test_file_path()
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 Reddit 热点话题 Tab
    with gr.Tab("Reddit 热点话题"):
        gr.Markdown("## Reddit 热点话题")  # 添加小标题

        # 创建 Radio 组件
        model_type = gr.Radio(["openai"], label="模型类型", info="使用 OpenAI GPT API")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_reddit_topic, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])
